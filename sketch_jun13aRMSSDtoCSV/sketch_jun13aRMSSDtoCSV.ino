...
void loop() { 
  ...
  if (!hrvStarted && numRRDetected >= 2) { // starts original 
//  if (!hrvStarted && numRRDetected >= 2 OR hrvStarted && ((millis() - hrvStartTime(i))==30*1000)) { // starts original 
    hrvStarted = true;
    hrvStartTime = millis();
//    hrvStartTime(i) = millis(); //every 30 secs new one starts
  }
...
//    rrDiffSquaredFiveMins = 
  ...
  Serial.print(millis() - hrvStartTime);
  Serial.print(",");
